...
CREATE TABLE `classrooms` (
  ...
  `building` ENUM('Д', 'В') NOT NULL COMMENT 'Корпус: Д-Джамбула, В-Вознесенский',
  `room_type` ENUM('компьютерный класс', 'лаборатория', 'лекционная аудитория') NOT NULL COMMENT 'Тип помещения',
...
CREATE TABLE `teachers` (
  ...
  `department` ENUM(
    ...
    'ИиУС',
    'ПОиУ'
  ) DEFAULT NULL COMMENT 'Кафедра',
  `employment_type` ENUM('штатный', 'внешний совместитель', 'внутренний совместитель', 'ГПХ') DEFAULT NULL COMMENT 'Форма занятости',
...
CREATE TABLE `schedule` (
  ...
  `numerator_denominator` ENUM('числитель', 'знаменатель') DEFAULT NULL COMMENT 'Числитель/знаменатель',
  ...
  `lesson_type` ENUM('лекц.', 'прак.', 'лаб.', 'конс.', 'экз.', 'ДО') DEFAULT NULL COMMENT 'Вид занятия',
  `is_occupied` TINYINT(1) DEFAULT 0 COMMENT 'Занята/свободна',
  `transfer_cancel` ENUM('нет', 'перенос', 'отмена') DEFAULT 'нет' COMMENT 'Перенос/отмена занятия',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE `software` (
  ...
  `building` ENUM('Д', 'В') DEFAULT NULL COMMENT 'Корпус',
