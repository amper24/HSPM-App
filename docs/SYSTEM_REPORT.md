...
**Стек:** PHP 7.2, MySQL 5.7, Vanilla JS, Docker Compose  
...
├── js/
│   ├── app.js              # **Фронтенд SPA** (Vanilla JS, без фреймворков, ~766 строк)
...
**Архитектура:** SPA на Vanilla JS (без фреймворков).  
**CDN:** axios 1.6.7 (внешняя зависимость).  
...
| Преподаватели | `teachers` | 8 колонок, ФИО склеен, фильтры по поиску/кафедре/степени/званию/занятости |
| Аудитории | `classrooms` | 7 колонок, булевы поля как Yes/No badge, фильтры по корпусу/типу/проектору/колонкам/местам |
...
**БД (23/23 ✅):** `tests/test_db.php` — подключение, таблицы, структура, индексы, FK, каскадное удаление.  
